...
def parse_args():
    ...
    parser.add_argument('--num_neurons_hidden', type=int, default=100, help='number of neurons for BNN')
    ...
    parser.add_argument('--m_te', type=int, default=1, help='number of multisample for test')
    parser.add_argument('--t', type=float, default=1, help='t value for log-t')
    parser.add_argument('--beta', type=float, default=0.01, help='beta for KL term')
...
def main_freq(args):
    ...
    np.save('logs/ECE_VAL' + str(args.t) + '_m_' + str(args.m) + '_eps_' + str(args.eps_tr) + '_freq.npy', ECE)
    np.save('logs/ACC_' + str(args.t) + '_m_' + str(args.m) + '_eps_' + str(args.eps_tr) + '_freq.npy', ACC)
    np.save('logs/LOG_' + str(args.t) + '_eps_' + str(args.eps_tr) + '_freq.npy', [LOG_NLL, LOG_NLLT, LOG_ACC])

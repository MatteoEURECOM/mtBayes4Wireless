...
def test_snrs(args, bnn, test_loader):
    ...
    for snr in test_loader.dataset.snrs:
        ...
        # estimate classes
        test_Y_i_hat = bnn(torch.from_numpy(test_X_i), args.m_te)
...
def test_with_ece_tfp_snrs(args, bnn, test_loader):
    ...
    for snr in test_loader.dataset.snrs:
        ...
        test_X_i = test_loader.dataset.X[np.where(np.array(test_loader.dataset.test_SNRs) == snr)]
        test_Y_i = test_loader.dataset.Y[np.where(np.array(test_loader.dataset.test_SNRs) == snr)]
        test_X_is=np.array_split(test_X_i,10)
        test_Y_is = np.array_split(test_Y_i, 10)
        test_Y_i_hat=[]
        true_pred=[]
        for X,y in zip(test_X_is,test_Y_is):
            test_Y_i_hat.append(bnn(torch.from_numpy(X), args.m_te).detach().numpy())
            true_pred.append(np.argmax(y,axis=1))
        ECE.append(tfp.stats.expected_calibration_error(args.num_bin, logits=np.concatenate(test_Y_i_hat), labels_true=np.concatenate(true_pred)).numpy())
    print(ECE)
...
def test_with_ece(args, bnn, test_loader):
    ...
    with torch.no_grad():
        for batch_idx, (data, target) in enumerate(test_loader):
            data, target = data.to(args.device), target.to(args.device)
            avg_prob = bnn(data, args.m_te) # m: number of multisamples
            #pred = avg_prob.argmax(dim=1, keepdim=True)

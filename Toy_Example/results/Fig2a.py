...
data=np.load('clean_datapoints.npy')
test=np.load('m=1t=1beta=1.npy')
i=0
m=1
lgds=[]
plots=[]
...
plt_s, =axs[0].plot(np.linspace(-10, 1, 1000),(999./11)*np.load('freq1.npy'),label=r'$\hat{\mathcal{R}}$',linestyle='-.',color='black')
plots.append(plt_s)
...
beta=1
plt_s, = axs[0].plot(np.linspace(-10, 1, 1000),(999./11)*np.load('m='+str(m)+'t=1beta='+str(beta)+'.npy'), color='tab:blue', label=r'$\mathcal{J}$')
plots.append(plt_s)
lgds.append(r'Bayesian $\beta=' + str(beta) + '$')

for beta in [0.1]:
    plt_s, = axs[0].plot(np.linspace(-10, 1, 1000),(999./11)*np.load('m='+str(m)+'t=1beta='+str(beta)+'.npy'),color=colors[i])
    plots.append(plt_s)
    lgds.append(r'Bayesian $\beta=' + str(beta) + '$')
    i=i+1
beta=1
plt_s, = axs[0].plot(np.linspace(-10, 1, 1000),(999./11)*np.load('m=10t=1beta='+str(beta)+'.npy'), color='tab:red', label=r'$\mathcal{J}$')
plots.append(plt_s)
...
plt_s, =axs[1].plot(np.argmax(np.load('posterior_freq1.npy'))*(11/999)-10,1, linestyle = 'None',marker='o',label=r'$\hat{\mathcal{R}}$',color='black')
plots.append(plt_s)
lgds.append(r'Frequentist')
plt.vlines(np.argmax(np.load('posterior_freq1.npy'))*(11/999)-10,0,1,color='black')
plt_s, =axs[1].plot(np.linspace(-10, 1, 1000),(999./11)*np.load('posterior_m=1t=1beta=1.npy')/np.sum(np.load('posterior_m=1t=1beta=1.npy')), color='tab:blue', label=r'$\mathcal{J}$')
plots.append(plt_s)
lgds.append(r'Bayesian $\beta=1$')
for beta in [0.1]:
    plt_s, =axs[1].plot(np.linspace(-10, 1, 1000),(999./11)*np.load('posterior_m='+str(m)+'t=1beta='+str(beta)+'.npy')/np.sum(np.load('posterior_m='+str(m)+'t=1beta='+str(beta)+'.npy')),color=colors[i])
    plots.append(plt_s)
    lgds.append(r'Bayesian $\beta=' + str(beta) + '$')
    i=i+1
beta=1
ciao=np.load('posterior_m=10t=1beta='+str(beta)+'.npy')

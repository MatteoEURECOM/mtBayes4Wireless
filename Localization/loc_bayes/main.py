...
def parse_args():
    ...
    parser.add_argument('--eps', type=float, default=0, help='Contamination Ratio')
...
def main(args):
    ...
    for rep in range(0, MC_reps):
        bnn = BNN(num_neurons_hidden=args.num_neurons_hidden, in_shape=train_dataset[:][0].shape[1]).to(args.device)
        args.m_te = 1
        ...
        torch.save(bnn, 'saved_models/REP_' + str(rep)+'_t_' + str(args.t) + '_m_' + str(args.m) + '_eps_' + str(args.eps) + '_dataset_' + str(args.dataset))
        bnn = torch.load('saved_models/REP_' + str(rep)+'_t_' + str(args.t) + '_m_' + str(args.m) + '_eps_' + str(args.eps) + '_dataset_' + str(args.dataset), map_location=torch.device('cpu'))
        TEST_MSE.append(mse_avg(args, bnn, test_loader,METER))
    if(METER):
        np.save('logs/MSE_AVG_Temp_' + str(args.t) + '_m_' + str(args.m) + '_eps_' + str(args.eps) + '_dataset_' + str(args.dataset) + '_METER.npy', TEST_MSE)
    else:
        np.save('logs/MSE_AVG_Temp_' + str(args.t) + '_m_' + str(args.m) + '_eps_' + str(args.eps) + '_dataset_' + str(args.dataset) + '.npy', TEST_MSE)
    np.save('logs/LOG_Temp_' + str(args.t) + '_m_' + str(args.m) + '_eps_' + str(args.eps) + '_dataset_' + str(args.dataset) + '.npy', [LOG_NLL, LOG_NLLT, LOG_MSE])
